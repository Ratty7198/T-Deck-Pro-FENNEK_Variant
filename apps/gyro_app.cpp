...
// Loading sequence: first draw the ‘Loading’ screen (separate tick), then the ~10 s
// blocking firmware upload — otherwise the image freezes without any indication.
...
class GyroApp : public App {
 public:
  ...
  void draw(Adafruit_GFX& g) override {
    ...
    gui::printAt(g, 10, TOP + 6, "IMU / Location", 3);

    if (s_phase == NEED_LOAD || s_phase == LOADING) {
      drawCentered(g, TOP + 90, "BHI260 Firmware laedt", 2);
      drawCentered(g, TOP + 118, "(~10 s, one-off)", 1);
    } else if (s_phase == FAILED) {
      drawCentered(g, TOP + 100, "Sensor not available", 2);
    } else {
      const gyro::Data& d = gyro::current();
      char l[40];
      g.setTextSize(1);
      g.setCursor(14, TOP + 44);  gui::print(g, "Acceleration (g)");
      g.setTextSize(2);
      ...
      g.setTextSize(1);
      g.setCursor(14, TOP + 130); gui::print(g, "Rotation rate (Grad/s)");
      g.setTextSize(2);
