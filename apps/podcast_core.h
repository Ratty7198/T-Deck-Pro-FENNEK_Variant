...
// =============================================================================
// podcast_core.h — Arduino-free RSS parser for the podcast app.
//
// Deliberately header-only and without Arduino/SD/WiFi: so it can be tested on the host (see
// tools/host_test_apps.cpp; precedence: mathquiz_core/flashcards_core).
//
// Podcast feeds list episodes in reverse chronological order —
// so the FIRST <item> is the latest episode. That’s exactly what we need
// (“always keep only the latest episode”). The caller (services/podcast)
// therefore does not need to load the entire feed (often >1 MB in size), but only
// enough from the beginning until the first <item> provides the three fields.
//
// The search is performed using a substring (no XML parser required), and is tolerant of
// upper and lower case, CDATA and common XML entities.
// =============================================================================
#pragma once
