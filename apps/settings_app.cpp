...
enum RowId {
  ...
  ROW_LANG, ROW_STANDBY, ROW_FONT, ROW_TZ, ROW_TIME,
  ROW_WLAT, ROW_WLON,
  ROW_WSSID, ROW_WPASS, ROW_WDEL,
  ...
  ROW_CALAUTO, ROW_CALUSER, ROW_CALPASS,
  ROW_COUNT
};

// --- Kategorien ("Ordner") ------------------------------------------------------
enum { CAT_RADIO, CAT_SYSTEM, CAT_TIME, CAT_WEATHER, CAT_WIFI, CAT_NAV, CAT_AI, CAT_OTA,
...
const char* catName(int c) {
  switch (c) {
    ...
    case CAT_TIME:     return "Clock";
    case CAT_WEATHER:  return "Weather";
    case CAT_WIFI:     return "WLAN";
    case CAT_NAV:      return "Navidrome";
    case CAT_AI:       return "AI Models";
    case CAT_OTA:      return "Update";
    case CAT_TODO:     return "Todo";
    case CAT_CAL:      return "Calender";
  }
  return "";
}
...
const char* rowName(int row) {
  ...
  switch (row) {
    ...
    case ROW_TZ:       return "Timezone";
    case ROW_TIME:     return "Time";
    case ROW_WLAT:     return "Weather Lat";
    case ROW_WLON:     return "Weather Lon";
    case ROW_WSSID:    return i18n::tr(Str::LblWifiSsid);
    case ROW_WPASS:    return i18n::tr(Str::LblWifiPass);
    case ROW_WDEL:     return "Delete";
    case ROW_NAVON:    return "Scrobbling";
    case ROW_NAVURL:   return "Server";
    case ROW_NAVUSER:  return "User";
    case ROW_NAVPASS:  return "Password";
    case ROW_AION:     return "Writing";
    case ROW_AIURL:    return "Server";
    case ROW_AIMODEL:  return "Model";
    case ROW_OTAVER:   return "Version";
    case ROW_OTAGO:    return "Refresh";
    case ROW_OTAURL:   return "Which";
    case ROW_TODOON:   return "Sync";
    case ROW_TODOURL:  return "Server";
    case ROW_TODOUSER: return "User";
    case ROW_TODOPASS: return "Password";
    case ROW_TODOPATH: return "Pfad";
    case ROW_TODOAUTO: return "Auto-Sync";
    case ROW_CALAUTO:  return "Auto-Sync";
    case ROW_CALUSER:  return "CalDAV-User";
    case ROW_CALPASS:  return "CalDAV-Pass";
  }
  return "";
}
...
void rowValue(int row, char* v, size_t n) {
  ...
  switch (row) {
    ...
    case ROW_TIME:
      ...
      break;
    case ROW_WLAT: {
      double lat = 52.52; settings::weatherPos(&lat, nullptr);
      if (s_edit == ROW_WLAT) snprintf(v, n, "%s_", s_editBuf);
      else snprintf(v, n, "%.5f", lat);
      break;
    }
    case ROW_WLON: {
      double lon = 13.41; settings::weatherPos(nullptr, &lon);
      if (s_edit == ROW_WLON) snprintf(v, n, "%s_", s_editBuf);
      else snprintf(v, n, "%.5f", lon);
      break;
    }
    case ROW_WSSID:
    ...
    case ROW_OTAGO:
      ...
      else                snprintf(v, n, "Please check");
      break;
    case ROW_OTAURL:
    ...
    case ROW_CALAUTO:
      ...
      break;
    case ROW_CALUSER:
      if (s_edit == ROW_CALUSER) snprintf(v, n, "%s_", s_editBuf);
      else { settings::calDavUser(v, n); if (!v[0]) snprintf(v, n, "-"); }
      break;
    case ROW_CALPASS:
      if (s_edit == ROW_CALPASS) snprintf(v, n, "%s_", s_editBuf);
      else { char pw[65]; settings::calDavPass(pw, sizeof(pw)); snprintf(v, n, "%s", pw[0] ? "****" : "-"); }
      break;
  }
}
...
void startEdit(int row) {
  ...
  switch (row) {
    ...
    case ROW_WLAT: {
      double lat = 52.52; settings::weatherPos(&lat, nullptr); snprintf(s_editBuf, sizeof(s_editBuf), "%0.5f", lat); break;
    }
    case ROW_WLON: {
      double lon = 13.41; settings::weatherPos(nullptr, &lon); snprintf(s_editBuf, sizeof(s_editBuf), "%0.5f", lon); break;
    }
    ...
    case ROW_CALUSER:  settings::calDavUser(s_editBuf, sizeof(s_editBuf)); break;
    case ROW_CALPASS:  settings::calDavPass(s_editBuf, sizeof(s_editBuf)); break;
    case ROW_TIME: {
...
void finishEdit(bool save) {
  if (save) {
    switch (s_edit) {
      ...
      case ROW_WLAT: {
        double lat = atof(s_editBuf); settings::setWeatherPos(lat, 13.41); break;
      }
      case ROW_WLON: {
        double lon = atof(s_editBuf); double lat = 52.52; settings::weatherPos(&lat, nullptr); settings::setWeatherPos(lat, lon); break;
      }
      case ROW_WSSID:
        if (s_wifiAdding) {
      ...
      case ROW_CALUSER:  settings::setCalDavUser(s_editBuf); break;
      case ROW_CALPASS:  settings::setCalDavPass(s_editBuf); break;
      case ROW_TIME: {
...
void onKey(char k) {
  ...
  if (s_edit >= 0) {
    ...
    switch (s_edit) {
      ...
      case ROW_WLAT:
      case ROW_WLON:     maxLen = 32;  break;
      case ROW_WPASS:
      case ROW_NAVPASS:
      case ROW_TODOPASS:
      case ROW_CALPASS:  maxLen = 64;  break;
      case ROW_NAVUSER:
      case ROW_TODOUSER:
      case ROW_CALUSER:  maxLen = 63;  break;
...
class SettingsApp : public App {
 public:
  ...
  void draw(Adafruit_GFX& g) override {
    ...
    else if (s_cat < 0)            gui::print(g, "Open Source (FOSS)");
    else if (inWifiList())         gui::print(g, wifiIsNewRow(s_sel) ? "Press Enter to connect to Wi-Fi": "Press Enter to open");
    else if (curRow() == ROW_WDEL) gui::print(g, "Enter deletes");
    else if (curRow() == ROW_OTAGO) gui::print(g, s_otaReady ? "Enter installed" : "Enter checking");
