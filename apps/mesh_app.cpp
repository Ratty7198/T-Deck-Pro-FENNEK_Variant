...
// News section (text size 1, line height 10 px)
...
// CHATS  = scrollable list: all channels + contacts WITH chat history (entry point).
// CONTACTS = all known nodes (adverts) for starting new DMs.
// CHANNEL/DM = open conversation. ERROR_SCREEN = Radio not available.
// JOIN_CHANNEL = Input field for a new hashtag channel (Name → joinHash).
...
// List layout depends on the screen: CHATS has a status bar above it.
int listTop()     { return (s_screen == CHATS) ? (LIST_Y + STATUS_H) : LIST_Y; }
int visibleRows() { return (s_screen == CHATS) ? 6 : 7; }

// --- Contact history: are there any DMs (on/off) with this contact? ---------------
...
// --- Chat list: Channels (first) + Contacts WITH chat history (as DMs) -------------
...
// CHATS also displays a fixed ‘+ New channel’ line at position 0.
...
// --- Time utilities --------------------------------------------------------------
// Local time (time zone set via timesync::applyTimezone) — the clock itself
// runs internally in UTC.
...
// --- Wrap history into display lines (window-based approach to scrollback) -------
// Only lines within the visible window [winStart, winStart+MSG_LINES) are saved;
// gidx counts ALL lines (for the total count during the counting pass, winStart=INT_MAX).
struct LineWindow {
...
// Format a message for display without timestamps.
void buildLine(const mesh_client::MsgView& m, char* out, size_t n) {
  if (s_screen == CHANNEL) {
    char tmp[224];
    snprintf(tmp, sizeof(tmp), "%s", m.text ? m.text : "");
    if (tmp[0] == '~') {
  ...
  if (m.kind == 2) {
    snprintf(out, n, "~%s: %s%s", i18n::tr(i18n::Str::MeshMe), m.text, ack);
  } else {
    snprintf(out, n, "%s: %s", m.from, m.text);
  }
}

// Scroll through the history of the current screen (channel or DM contact).
...
void drawCompose(Adafruit_GFX& g) {
  ...
  // Only display the end if it is too long.
  ...
  gui::print(g, show);
  // If the line is empty and the DM has failed: prompt to resend (on the right).
...
// Input box for the channel name: fixed '#' prefix (unless the user types
// one in themselves), to make it clear that a hashtag channel is being created.
...
void drawChannelOrDm(Adafruit_GFX& g) {
  if (s_screen == CHANNEL) {
    ...
    if (cn[0] == '#') memmove(cn, cn + 1, strlen(cn));
    drawHeaderBar(g, cn);
  } else {
    char t[48];
  ...
  // Iteration 1: Determine the total number of lines (do not save).
  LineWindow w;
  ...
  // Iteration 2: collect only the viewport.
  ...
  for (int i = 0; i < w.count; i++) {
    ...
    bool isOwnMsg = (line[0] == '~');
    const char* text = isOwnMsg ? (line + 1) : line;
    ...
    size_t nameLen = (size_t)(colon - text);
    char name[32];
    if (nameLen >= sizeof(name)) nameLen = sizeof(name) - 1;
    memcpy(name, text, nameLen);
    name[nameLen] = '\0';
    ...
    g.setCursor(7, y);
    gui::print(g, name);
    g.setCursor(6 + (int)strlen(name) * 6 + 4, y);
    gui::print(g, rest);
...
// Right-hand scroll bar: ▲/▼ indicators if there are more entries above or below.
...
void drawChats(Adafruit_GFX& g) {
  ...
  int n = chatsRowCount();   // including the ‘+ New channel’ line at position 0

  // Placeholder text below the ‘Add’ line if there are no actual chats yet.
  if (nChats == 0) {
...
void drawContacts(Adafruit_GFX& g) {
  ...
  for (int r = 0; r < vis && s_off + r < n; r++) {
    ...
    // Details (Hops + Last seen) – small, right-aligned on the same line.
...
// “+ New channel”: open an empty input field for the hashtag name.
...
// Join the name entered (PSK derived from #name, persisted on SD).
// Success → jump directly to the new channel; failure → return to the list.
void doJoinChannel() {
  if (!s_compose[0]) return;
...
// Open a chat list entry (channel or DM).
...
// Scroll the list by one page (touch scroll bar). dir: -1 up, +1 down.
...
void onTouch(int x, int y) {
  ...
  // 'Home' now appears in the status bar; only the Mesh-Top (CHATS) has kBtn3 = Back.
  ...
  if (s_screen == JOIN_CHANNEL) {
    ...
    return;   // Go to Home via the status bar; otherwise, ignore the tap
  }

  // CHANNEL / DM: back to the list, advert, scroll back.
  if (kBtn1.hit(x, y)) { backToChats(); return; }
  if (kBtn2.hit(x, y)) { mesh_client::sendAdvert(); markDirty(); return; }
...
void onKey(char k) {
  ...
  // JOIN_CHANNEL: Type the channel name using the keyboard; press Enter to join.
  if (s_screen == JOIN_CHANNEL) {
  ...
  // CHANNEL/DM: The keyboard types into the input line.
  if (k == '\r') {
  ...
  if (k == '\b') {
    int len = strlen(s_compose);
    if (len > 0) { s_compose[len - 1] = '\0'; markDirty(); }
    else { backToChats(); }   // blank input + Backspace = return to the chat list
    return;
  }
  if (k >= 32 && k < 127) {
    int len = strlen(s_compose);
 ...
 public:
  ...
  void onEnter() override {
    if (!s_initTried) {
      ...
      s_screen = CHATS;   // Returning to the app always displays the chat overview
    }
    s_msgScroll = 0;
    s_seenChanges = mesh_client::changeCounter();
  }
  ...
  // The Mesh pump is ALWAYS running (even when other apps are in the foreground).
  ...
  void tick() override {
    ...
    // New messages/contacts -> redraw (coalesced via the dirty flag).
