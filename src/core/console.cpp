...
// Initialize mesh if needed (for send commands without UI).
bool ensureMesh() {
  if (mesh_client::ready()) return true;
  Serial.println("[CON] Mesh not initialized — starting ...");
...
void cmdHelp() {
  ...
  Serial.println("[CON]   status            - System/Mesh status");
  Serial.println("[CON]   time              - Time/quality/source (time sync)");
  Serial.println("[CON]   time set <...>    - Set time (YYYY-MM-DD HH:MM, local)");
  Serial.println("[CON]   time sync         - Force NTP sync now (WiFi)");
  Serial.println("[CON]   tz <POSIX-TZ>     - Set timezone (default Europe/Berlin)");
  Serial.println("[CON]   sleep             - Enter standby (like long press)");
  Serial.println("[CON]   alarm             - List alarms");
  Serial.println("[CON]   alarm <i> <hh:mm> [days] - Set alarm (days: once|daily|mon,tue,..)");
  Serial.println("[CON]   alarm <i> off / <i> mode sound|blink|vibra|both|all / sound <path> / test / stop / snooze");
  Serial.println("[CON]   mesh init         - Initialize mesh radio");
  Serial.println("[CON]   mesh eco [on|off] - RX duty cycle (radio power save) show/set");
  Serial.println("[CON]   advert            - Send zero-hop advert (with battery telemetry)");
  Serial.println("[CON]   advert flood      - Send flood advert (multi-hop, reaches distant nodes)");
  Serial.println("[CON]   pos [<lat> <lon>] - Show/set node position (location beacon in advert)");
  Serial.println("[CON]   gps [sek] / off   - GPS test: read raw NMEA + fix (default 10 s), module off");
  Serial.println("[CON]   public <text>     - Send message to public channel");
  Serial.println("[CON]   dm <idx> <text>   - Direct message to contact #idx");
  Serial.println("[CON]   contacts          - List contacts");
  Serial.println("[CON]   contacts reset    - Clear all contacts (rebuild from adverts)");
  Serial.println("[CON]   channels          - List joined channels");
  Serial.println("[CON]   join <name>       - Join hashtag channel (e.g. join test)");
  Serial.println("[CON]   chan <name> <txt> - Send message to hashtag channel");
  Serial.println("[CON]   msgs              - Dump message history");
  Serial.println("[CON]   meshlog           - Show end of SD message log");
  Serial.println("[CON]   battlog           - Dump battery/activity log (BATTLOG)");
  Serial.println("[CON]   gauge             - Dump BQ27220 registers (RM/FCC/SoC, read-only)");
  Serial.println("[CON]   i2cscan           - Probe I2C bus (RTC at 0x51?)");
  Serial.println("[CON]   ls <path>         - List SD directory (e.g. ls /books)");
  Serial.println("[CON]   rm <path>         - Recursively delete file/folder (including long Calibre paths)");
  Serial.println("[CON]   books             - Rescan books + dump");
  Serial.println("[CON]   wifi list         - List known WiFi networks");
  Serial.println("[CON]   wifi ssid <name>  - Set WiFi SSID for profile 0 (NVS)");
  Serial.println("[CON]   wifi pass <pw>    - Set WiFi password for profile 0 (NVS)");
  Serial.println("[CON]   wifi add <name> [pw] - Add another WiFi profile");
  Serial.println("[CON]   wifi del <n>      - Delete WiFi profile n");
  Serial.println("[CON]   wifi status       - WiFi/webserver status");
  Serial.println("[CON]   wifi start        - Start web file manager");
  Serial.println("[CON]   wifi stop         - Stop web file manager");
  Serial.println("[CON]   nav url <url>     - Set Navidrome server URL");
  Serial.println("[CON]   nav user <name>   - Set Navidrome user");
  Serial.println("[CON]   nav pass <pw>     - Set Navidrome password");
  Serial.println("[CON]   nav on|off        - Toggle scrobbling");
  Serial.println("[CON]   nav test          - Subsonic ping (WiFi)");
  Serial.println("[CON]   scrobble          - Show pending scrobbles");
  Serial.println("[CON]   scrobble flush    - Upload pending scrobbles now (WiFi)");
  Serial.println("[CON]   ollama url <url>  - Set Ollama server (e.g. http://10.0.0.5:11434)");
  Serial.println("[CON]   ollama model <m>  - Set Ollama model (e.g. llama3.2)");
  Serial.println("[CON]   ollama on|off     - Auto-polish notes");
  Serial.println("[CON]   ollama / test     - Status / connection test (WiFi)");
  Serial.println("[CON]   ollama flush      - Polish pending notes now (WiFi)");
  Serial.println("[CON]   podcast           - Show feeds + local episodes");
  Serial.println("[CON]   podcast feed <url>- Subscribe to feed (feeds.txt)");
  Serial.println("[CON]   podcast rm <idx>  - Remove feed");
  Serial.println("[CON]   podcast on|off    - Toggle auto-sync before standby");
  Serial.println("[CON]   podcast sync      - Download latest episodes now (WiFi)");
  Serial.println("[CON]   calibre           - Calibre sync status");
  Serial.println("[CON]   calibre url <url> - Set Calibre-Web (e.g. https://calibre.example)");
  Serial.println("[CON]   calibre user/pass - Calibre-Web login (OPDS basic auth)");
  Serial.println("[CON]   calibre shelf <s> - Book shelf, name or ID (default Fennek)");
  Serial.println("[CON]   calibre on|off    - Toggle auto-sync before standby");
  Serial.println("[CON]   calibre sync      - Download new books now (WiFi)");
  Serial.println("[CON]   ota               - OTA status (version + update URL)");
  Serial.println("[CON]   ota url <url>     - Set update source (GitHub release API/manifest)");
  Serial.println("[CON]   ota check         - Check for new firmware (WiFi)");
  Serial.println("[CON]   ota update|force  - Download + flash + reboot firmware (WiFi)");
  Serial.println("[CON]   settings show     - Show current settings");
  Serial.println("[CON]   settings save     - Save settings to /fennek.ini (SD)");
  Serial.println("[CON]   settings load     - Load /fennek.ini into NVS");
}

void cmdSettingsShow() {
  // exportIni() deliberately omits WiFi password (field remains empty),
  // so there's nothing to mask here.
...
void cmdWifiStatus() {
  ...
  const char* st = "off";
  switch (webfm::state()) {
    case webfm::State::CONNECTING: st = "connecting ..."; break;
    case webfm::State::RUNNING:    st = "running"; break;
    case webfm::State::FAILED:     st = "FAILED"; break;
    default: break;
  }
  char ip[16];
  webfm::ipStr(ip, sizeof(ip));
  Serial.printf("[CON] WiFi: SSID='%s' Password=%s | Profiles: %d | Status: %s%s%s | Requests=%lu\n",
                ssid[0] ? ssid : "(not set)",
                pass[0] ? "set" : "(not set)", settings::wifiCount(), st,
                ip[0] ? " http://" : "", ip,
                (unsigned long)webfm::requestCount());
}

// List SD directory (diagnostic, non-recursive).
void cmdLs(const char* path) {
  if (!board::sdReady()) { Serial.println("[CON] No SD card"); return; }
  spiLock();
  File d = SD.open(path);
  if (!d) {
    spiUnlock();
    Serial.printf("[CON] '%s' does not exist\n", path);
    return;
  }
  if (!d.isDirectory()) {
    Serial.printf("[CON] %s is a file (%u bytes)\n", path, (unsigned)d.size());
    d.close();
    spiUnlock();
    return;
  }
  ...
  d.close();
  spiUnlock();
  Serial.printf("[CON] %d entries in %s\n", n, path);
}

// Recursive delete for console. Uses large path buffer (512 instead of
// 192-byte WebFM buffer) because Calibre filenames can push paths >240 chars.
// Individual path components stay under FATFS limit (255); only the fixed
// firmware buffers were too small. In arduino-esp32 2.0.x, f.name() returns
// the basename, so we reassemble the path here. Must be called under spiLock!
...
// Show last ~2 KB of SD message log.
void cmdMeshLog() {
  if (!board::sdReady()) { Serial.println("[CON] No SD card"); return; }
  spiLock();
  if (!SD.exists("/meshcore/messages.log")) {
    spiUnlock();
    Serial.println("[CON] /meshcore/messages.log does not exist (yet)");
    return;
  }
  File f = SD.open("/meshcore/messages.log");
  if (!f) { spiUnlock(); Serial.println("[CON] Log not readable"); return; }
  uint32_t sz = f.size();
  ...
  f.close();
  spiUnlock();
  if (rd <= 0) { Serial.println("[CON] Log empty"); return; }
  buf[rd] = '\0';
  Serial.printf("[CON] /meshcore/messages.log (%u bytes, last %d):\n", (unsigned)sz, rd);
  Serial.println(buf);
}
...
// Display arbitrary SD text file in chunks (SD read under spiLock, serial after).
