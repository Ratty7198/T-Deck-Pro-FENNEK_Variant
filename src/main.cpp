...
#include "apps/wifi_monitor_app.h"
#include "apps/node_monitor_app.h"
#include "core/board.h"
#include "core/display.h"
...
void setup() {
  ...
  Serial.println("\n[JARVIS] Boot — JARVIS " FENNEK_VERSION " (T-Deck Pro)");
  // Diagnose Standby-Sofort-Wake: Warum sind wir hier? (Reset-Grund + Wake-Quelle)
  Serial.printf("[JARVIS] Reset reason=%d Wake source=%d ext1=0x%llx\n",
  ...
  board::initBus();
  Serial.println("[JARVIS] Power + Buses are fine");

  // 2) Display zuerst — Nutzer sieht sofort etwas.
  display::begin();
  Serial.println("[JARVIS] Display init ok");

  // Begrüßungsscreen (Badge/Titel/Frequenz) — einmalig beim Kaltstart, bevor
  // die restliche Initialisierung (SD/Bibliothek/Audio) den Bus belegt.
  // full=true: Ghosting-Reset nach dem Schlafbild bzw. leerem Panel.
  display::render(bootsplash::draw, /*full=*/true);
  Serial.println("[JARVIS] Bootsplash shown");
  delay(2500);   // Begrüßungsscreen sichtbar lassen, bevor Boot weitergeht

  // 3) Eingabe + Battery + Settings (alles unabhängig vom SPI-Bus).
  bool touchOk = touch::begin();
  Serial.printf("[JARVIS] Touch init: %s\n", touchOk ? "ok" : "FAILED");
  bool kbOk = keyboard::begin();
  Serial.printf("[JARVIS] Keyboard (TCA8418): %s\n", kbOk ? "ok" : "NOT found");
  bool battOk = battery::begin();
  Serial.printf("[JARVIS] Battery (BQ27220): %s (%u mV, %u%%)\n",
                battOk ? "ok" : "NOT found",
  ...
  Serial.printf("[JARVIS] SD-Card: %s\n", sdOk ? "mounted" : "Not found / failed");
  if (sdOk) {
    // Einmalige Migration: Cache-Verzeichnis hieß früher /.meck (Meck-Ära).
    spiLock();
    if (SD.exists("/.meck") && !SD.exists("/.JARVIS")) {
      bool ok = SD.rename("/.meck", "/.JARVIS");
      Serial.printf("[JARVIS] SD-Cache-Migration /.meck -> /.JARVIS: %s\n", ok ? "ok" : "ERROR");
    }
    spiUnlock();
    ...
    if (settingsfile::importFromSd())
      Serial.println("[JARVIS] Settings loaded from /JARVIS.ini");
  }
  library::begin();
  int n = 0;
  if (sdOk) {
  ...
  Serial.printf("[JARVIS] Tracks found: %d%s\n", n,
                library::scanning() ? " (Scan is running in the background)" : "");

  // 5) Audio-Engine (startet den Decode-Task auf Core 0).
  audio::begin();
  if (settings::volume() != 255) audio::setVolume(settings::volume());
  Serial.printf("[JARVIS] Audio-Engine started (freer PSRAM: %u KB)\n",
  ...
  Serial.printf("[JARVIS] Artists=%d Albums=%d Tracks=%d Playlists=%d\n",
  ...
  appmgr::add(weather_app::get());
  appmgr::add(wifi_monitor_app::get());
  appmgr::add(node_monitor_app::get());
  launcher::setTile(3, i18n::Str::TileMusic,    music_app::get());
  launcher::setTile(1, i18n::Str::TileBook,     book_app::get());
  launcher::setTile(2, i18n::Str::TileReader,   reader_app::get());
  launcher::setTile(0, i18n::Str::TileMesh,     mesh_app::get());
  launcher::setTile(18, i18n::Str::TileSettings, settings_app::get());
  ...
  launcher::setTile(4, i18n::Str::TileCalendar, calendar_app::get());
  launcher::setTile(14, i18n::Str::TileTodo,     todo_app::get());
  launcher::setTile(15, i18n::Str::TilePodcast,  podcast_app::get());
  launcher::setTile(16, i18n::Str::TileWeather,  weather_app::get());
  launcher::setTile(17, i18n::Str::TileWiFi,     wifi_monitor_app::get());
  launcher::setTile(13, i18n::Str::TileNodeMon,  node_monitor_app::get());
  appmgr::begin();
  Serial.println("[Jarvis] Setup complete — Launcher is running.");
  ...
  gamesSmokeKey('d');              // -> Schwarz (JARVIS = Weiß beginnt)
  gamesSmokeKey('s');
  ...
  gamesSmokeKey('\r');
  gamesSmokeTap(48, 100);          // Zelle 0 setzen, JARVIS antwortet
  gamesSmokeKey('\b');             // zurück ins Menü
  ...
  Serial.println("[JARVIS] MESH_SMOKE_TEST: initialisiere Mesh ...");
  bool meshOk = mesh_client::begin();
  Serial.printf("[JARVIS] MESH_SMOKE_TEST: %s (freier Heap: %u KB)\n",
  ...
  if (n > 0) {
    ...
    for (int i = 0; i < 12; i++) {
      delay(2000);
      Serial.printf("[JARVIS] >>> erzwinge Display-Refresh #%d <<<\n", i + 1);
