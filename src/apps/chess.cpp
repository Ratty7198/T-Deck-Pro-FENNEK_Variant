...
void aiTaskFn(void*) {
  ...
  Serial.printf("[GAME] Chess AI: Depth %d, %lu Node, %lu ms, Score %d\n",
...
void startThinking() {
  ...
  if (xTaskCreatePinnedToCore(aiTaskFn, "chess_ai", 24576, nullptr, 1, nullptr, 1) != pdPASS) {
    s_thinking = false;
    Serial.println("[GAME] Chess AI: Task start FAILED");
  }
...
void trainerTaskFn(void*) {
  ...
  Serial.printf("[GAME] Trainer-Analyse: %lu ms, Loss %d cp, Class %d\n",
...
void startTrainerAnalysis() {
  ...
  if (xTaskCreatePinnedToCore(trainerTaskFn, "chess_tr", 24576, nullptr, 1, nullptr, 1) != pdPASS) {
    s_thinking = false;
    s_analyzing = false;
    Serial.println("[GAME] Trainer-Analyse: Task-Start FAILED");
  }
  appmgr::markDirty();
}
...
void newGame() {
  ...
  s_dirtySave = true;
  s_sub = BOARD;
  Serial.printf("[GAME] Chess: new game (%s, Level %s)\n",
                s_mode ? "2 players" : "against Jarvis", i18n::tr(kLevelName[s_level]));
  maybeStartAi();
  appmgr::markDirty();
}
...
bool loadGame() {
  ...
  refreshLegal();
  updateResult();
  Serial.println("[GAME] Chess: Game loaded from NVS");
  return true;
}
