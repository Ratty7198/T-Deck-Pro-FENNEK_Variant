...
// List bar: Back | Sync all | Up | Down
...
bool s_hasLocal[podcast::kMaxFeeds] = {};  // Cached outside draw(), never under spiLock
int  s_n = -1;                 // -1 = not yet loaded
int  s_sel = 0, s_off = 0;

int  s_openFeed = -1;          // Feed selected in DETAIL
podcast::Local s_local;
bool s_haveLocal = false;
uint32_t s_epKey = 0;          // CRC32 of the episode path (bookmark key)

// Blocking synchronisation: the request is only executed after the "Synchronise…" frame
// so that the user sees feedback before the UI is blocked.
int      s_syncReq = -2;       // -2 = none, -1 = all, >=0 = feed index
bool     s_syncDrawn = false;
char     s_syncFeedName[64] = "";
char     s_syncLog[64] = "";
int      s_syncPct = -1;       // -1 = undefined, 0–100 = download %
int      s_syncMark = 0;       // Pulse counter for chunked downloads (512 KB per chunk = +1)
uint32_t s_lastProgMs = 0;     // Drossel: latest E-Ink region update
...
void loadFeeds() {
  ...
  // localEpisode() requires spiLock — cache it here outside draw() (never call it within draw())
...
// Progress bar for renderRegion(): bar + percentage display.
// MUST NOT be called from within draw() (draw() already holds spiLock;
// renderRegion() would request the same lock → self-deadlock).
...
// Progress callback for podcast::syncAll/syncOne — called after spiUnlock(),
// so renderRegion() is safe here (no nested lock).
void onSyncProgress(const char* /*phase*/, int pct) {
  ...
  // Each partial refresh holds the HSPI bus for ~650 ms and blocks the
  // SD writes during the download for that duration (E-Ink ⊥ SD on the same bus). Refresh sparingly,
  // so that the refresh doesn’t eat up half the throughput — the user just needs
  // a sign of life, not a smooth progress bar.
  uint32_t now = millis();
...
void drawSyncing(Adafruit_GFX& g) {
  ...
  gui::printAt(g, 10, 110, "Synchronising ...", 2);
  if (s_syncFeedName[0]) gui::printAt(g, 10, 140, s_syncFeedName, 1);
  drawSyncProgressStrip(g);
  gui::printAt(g, 10, SYNC_STRIP_Y + SYNC_STRIP_H + 4, "WLAN active - please wait", 1);
}
...
void drawList(Adafruit_GFX& g) {
  ...
  if (s_n <= 0) {
    gui::printAt(g, 10, 80, "No Feeds", 2);
    gui::printAt(g, 10, 110, "/podcasts/feeds.txt", 1);
    gui::printAt(g, 10, 124, "Edit via WebFM", 1);
    gui::drawButton(g, kRetrySD, i18n::tr(i18n::Str::BtnRetrySD), false);
    return;
  }
...
// Word-oriented multi-line printing (episode titles can be long).
...
void drawDetail(Adafruit_GFX& g) {
  ...
  if (s_haveLocal && s_local.title[0]) {
    ...
    gui::printAt(g, 6, HEADER_H + 10, "No episode loaded", 2);
    gui::printAt(g, 6, HEADER_H + 34, "\"Sync\" loads the latest episode.", 1);
  }
  ...
  gui::drawButton(g, kBtnList, "List", false);
}
...
void requestSync(int idx) {
  ...
  else strcpy(s_syncFeedName, "All Feeds");
  markDirty();
}
...
class PodcastApp : public App {
 public:
  ...
  void handleInput(const InputEvent& e) override {
    if (s_syncReq != -2) return;   // No input during synchronisation
  ...
  void tick() override {
    // Deferred, blocking synchronisation: only after the "Synchronise…" frame.
    if (s_syncReq != -2) {
      if (!s_syncDrawn) {
        if (!appmgr::isDirty()) s_syncDrawn = true;   // Frame is in place
        return;
      }
      int req = s_syncReq;
    ...
    // Progress bar in the player (region refresh, not full screen).
