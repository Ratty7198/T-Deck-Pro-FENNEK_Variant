...
uint8_t  s_lang = 3;            // UI-Sprache (0 = Deutsch, s. i18n::Lang)
...
void migrateFromMeck() {
  ...
  if (old.isKey("vol")) {
    ...
    Serial.println("[FENNEK] NVS-Migration meck -> ProtocolXIX Completed");
  }
  old.end();
  Preferences wipe;
...
void begin() {
  ...
  s_lang = s_prefs.getUChar("lang", 3);
...
namespace {
bool   s_weatherLocLoaded = false;
double s_weatherLat = 52.52, s_weatherLon = 13.41;

void weatherEnsure() {
  if (s_weatherLocLoaded || !s_open) return;
  s_weatherLocLoaded = true;
  if (s_prefs.isKey("wlat")) s_weatherLat = s_prefs.getDouble("wlat", 52.52);
  if (s_prefs.isKey("wlon")) s_weatherLon = s_prefs.getDouble("wlon", 13.41);
}

}  // namespace

void weatherPos(double* lat, double* lon) {
  weatherEnsure();
  if (lat) *lat = s_weatherLat;
  if (lon) *lon = s_weatherLon;
}

void setWeatherPos(double lat, double lon) {
  weatherEnsure();
  if (!s_open) return;
  if (lat != s_weatherLat) { s_weatherLat = lat; s_prefs.putDouble("wlat", lat); }
  if (lon != s_weatherLon) { s_weatherLon = lon; s_prefs.putDouble("wlon", lon); }
}
...
void tzString(char* out, size_t n) {
  const char* def = "GMT0BST,M3.5.0/1,M10.5.0";
